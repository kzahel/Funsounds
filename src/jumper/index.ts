...
let gatewayWorld: WorldLayer | null = null;
...
function updateHud(): void {
  ...
  screenEl.dataset.rainbowGateLocked = String(rainbowGateLocked());
...
function weddingPartnerKind(): WeddingPartnerKind {
  return underwaterWorld || deepSeaWorld || gatewayWorld === 'sunkenCastle' ? 'fish' : 'buddy';
}
...
function currentWorldLayer(): WorldLayer {
  if (gatewayWorld) return gatewayWorld;
...
function arriveInWorld(world: WorldLayer): void {
  ...
  gatewayWorld = world === 'moonBase'
    || world === 'dinosaurJungle'
    || world === 'volcano'
    || world === 'sunkenCastle'
...
function worldGatewayDefinitions(world: WorldLayer): WorldGateway[] {
  ...
  if (world === 'surface') {
    return [
      { ...portalPositionAt(0.24), kind: 'vine', target: 'dinosaurJungle', locked: false },
      {
        ...portalPositionAt(0.54),
        kind: 'rainbowGate',
  ...
  if (world === 'underwater') {
    return [{ ...portalPositionAt(0.7), kind: 'shellGate', target: 'sunkenCastle', locked: false }];
  }
  return [];
}

function rainbowGateLocked(): boolean {
  return !WORLD_LAYERS
    .filter((layer) => layer !== 'rainbowDreamland')
    .every((layer) => exploredWorlds.has(layer));
}
...
function noteBirdTripSide(side: BirdTripSide): void {
  if (!hasBuddyChain() || candyWorld || upperAtmosphereWorld || gatewayWorld || undergroundWorld || underwaterWorld || deepSeaWorld || birdTripSide === side) return;
  resetBirdTrip(side);
}

function updateBirdTripSide(cx: number): void {
  if (!hasBuddyChain() || candyWorld || upperAtmosphereWorld || gatewayWorld || undergroundWorld || underwaterWorld || deepSeaWorld || level.leftGoalX === undefined) return;
...
function canSpawnBird(): boolean {
  return hasBuddyChain() && !candyWorld && !upperAtmosphereWorld && !gatewayWorld && !undergroundWorld && !underwaterWorld && !deepSeaWorld && !birdRide && !weddingEvent && phase === 'playing';
}
...
function handlePitFall(): void {
  ...
  if (gatewayWorld === 'dinosaurJungle') {
    gatewayWorld = null;
    gatewayTravel = null;
    respawnTo(lastSafe);
    spawnDust(lastSafe.x + PLAYER_W / 2, lastSafe.y + PLAYER_H, 10);
  ...
  if (gatewayWorld === 'sunkenCastle') {
    gatewayWorld = null;
    underwaterWorld = true;
    gatewayTravel = null;
    respawnTo(lastSafe);
    spawnSparks(lastSafe.x + PLAYER_W / 2, lastSafe.y + PLAYER_H, 12);
    showToast('A bubble carried you to the reef!');
    speakText('Back to the coral reef!', { rate: 1.05, pitch: 1.2 });
    setStatus('Back in Coral Reef. Press Space beside the giant shell to return.');
    updateHud();
    return;
  }
  if (gatewayWorld === 'rainbowDreamland') {
    gatewayWorld = null;
    gatewayTravel = null;
    respawnTo(lastSafe);
    spawnSparks(lastSafe.x + PLAYER_W / 2, lastSafe.y + PLAYER_H, 18);
...
function update(dt: number): void {
  ...
  const res = underwaterWorld || deepSeaWorld || gatewayWorld === 'sunkenCastle'
    ? stepSwimBody(body, solids, { ...input, downHeld: lockForWeddingEvent ? false : downHeld }, dt)
    : stepBody(
      body,
      solids,
      input,
      gatewayWorld === 'moonBase' ? MOON_TUNING : gatewayWorld === 'rainbowDreamland' ? RAINBOW_TUNING : tuning,
      dt,
    );
