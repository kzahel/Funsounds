...
test('Buddy world map pauses play and revisits every layer without resetting snacks', async ({ page }) => {
  ...
  await page.addInitScript(() => {
    localStorage.setItem('barrelhop.worlds.explored', JSON.stringify([
      'surface', 'candy', 'upperAtmosphere', 'moonBase', 'dinosaurJungle', 'volcano', 'sunkenCastle', 'rainbowDreamland', 'toyRoom', 'cave', 'underwater', 'deepSea',
    ]));
  });
  ...
  await expect(map.locator('[data-jp-world]')).toHaveCount(12);
